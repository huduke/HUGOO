...
// 获取账户下的米粒及到期提醒,不带realCookie也能查
function GetMili(cookie, deviceId, blackBox) {
  return new Promise((resolve, reject) => {
    let options = {
      url: "https://fmapp.chinafamilymart.com.cn/api/app/member/v2/mili/quantity",
      headers: {
        "Accept": "*/*",
        "Accept-Encoding": "br;q=1.0, gzip;q=0.9, deflate;q=0.8",
...
//获取抽奖次数
function GetTurnCount(cookie) {
  return new Promise((resolve, reject) => {
    let options = {
      url: "https://fmapp.chinafamilymart.com.cn/api/app/market/turn/user",
      headers: {
        "Accept": "*/*",
    ...
    magicJS.post(options, (err, resp, data) => {
      if (err) {
        magicJS.logError(`获取抽奖次数失败，请求异常：${err}`);
        reject("❌获取抽奖次数失败，请求异常，请查阅日志！");
      } else {
        try {
          let obj = typeof data === "string" ? JSON.parse(data) : data;
          if (obj.code === "200") {
            ...
            magicJS.logError(`获取抽奖次数失败，响应异常：${data}`);
            reject("❌获取抽奖次数失败，响应异常，请查阅日志！");
          }
        } catch (err) {
          magicJS.logError(`获取抽奖次数失败，执行异常：${err}，接口响应：${data}`);
          reject("❌获取抽奖次数失败，执行异常，请查阅日志！");
        }
      }
    });
  });
...
//抽奖
function GetTurnLottery(cookie,blackBox) {
  return new Promise((resolve, reject) => {
    let options = {
      url: "https://fmapp.chinafamilymart.com.cn/api/app/market/turn/lottery",
      headers: {
        "blackBox": blackBox,
        "Accept": "*/*",
        "Origin" : "https://fmapp-activity.chinafamilymart.com.cn",
      ...
      if (err) {
        ...
        try {
          ...
          if (obj.code === "200") {
            ...
            magicJS.logError(`抽奖响应异常：${data}`);
            reject("❌抽奖响应异常，请查阅日志！");
          }
        } catch (err) {
          magicJS.logError(`抽奖失败，执行异常：${err}，接口响应：${data}`);
          reject("❌抽奖失败，执行异常，请查阅日志！");
        }
      }
    });
  });
...
(async () => {
  if (magicJS.isRequest && getCookieRegex.test(magicJS.request.url)) {
    ...
    if (!!!cookie || !!!deviceId) {
      ...
      let [checkInErr, [data, message]] = await magicJS.attempt(magicJS.retry(CheckInNewVerrsion, 3, 1000)(cookie, deviceId, blackBox,realCookie), []);
      if (checkInErr) {
        subTitle = checkInErr;
      } else if (data) {
        subTitle = `本日签到成功`;
      ...
      // 获取米粒
      let [getMiliErr, getMiliResult] = await magicJS.attempt(GetMili(cookie, deviceId, blackBox));
      if (!getMiliErr) {
        if (content) content += "\n";
      ...
      // 获取抽奖次数
      let turnTime = 0;
      let [getTurnErr, getTurnResult] = await magicJS.attempt(GetTurnCount(cookie));
      if (!getTurnErr) {
        if (content) content += "\n";
        turnTime = getTurnResult.ableCurrentTimes;
        if(turnTime > 0){
          content += `今天还有 ${getTurnResult.ableCurrentTimes} 抽奖机会`;
        }else{
          content += `今天没有抽奖机会`;
        }
      }
